...
class PrecomputedKernel(Kernel):
    ...
    Invocations of :meth:`compute` should specify as arguments two iterables
    of whose first item refers to an index of the above mentioned matrix:
    ...
    Specification of non iterable arguments raise a :exc:`TypeError`:

    >>> k.compute(0, 1)
    ...
    TypeError: 'int' object is unsubscriptable

    If the supplied arguments contain more than one element, these are ignored:
    ...
    Precomputed kernels are used in conjunction with *dummy* labeled samples
    whose patterns refer to indices on the kernel matrix:

    >>> from yaplf.data import LabeledExample
    >>> xor_sample = [LabeledExample([0], -1), LabeledExample([1], -1), \
    ... LabeledExample([2], 1), LabeledExample([3], 1)]
    >>> from yaplf.models.kernel import PrecomputedKernel
    >>> k = PrecomputedKernel(((9, 1, 4, 4), (1, 1, 1, 1), (4, 1, 4, 1), \
    ... (4, 1, 1, 4)))
    >>> from yaplf.algorithms.svm.classification import \
    ... SVMClassificationAlgorithm
    >>> from yaplf.algorithms.svm.solvers import CVXOPTClassificationSolver
    >>> s = CVXOPTClassificationSolver()
    >>> alg = SVMClassificationAlgorithm(xor_sample, kernel = k, solver = s)
    >>> alg.run()
    >>> alg.model
    SVMClassifier([1.9999999995368085, 3.3333333325135617, 2.6666666660251854,
    2.6666666660251854], -0.999999999697, [LabeledExample([0], -1.0),
    LabeledExample([1], -1.0), LabeledExample([2], 1.0), LabeledExample([3],
    1.0)], kernel = PrecomputedKernel(((9, 1, 4, 4), (1, 1, 1, 1),
    (4, 1, 4, 1), (4, 1, 1, 4))))
    >>> alg.model.test(xor_sample)
    0.0

    The class :class:`yaplf.data.PrecomputedKernelSample` can be used in order
    to automatically generate the labeled sample given its labels and the
    corresponding kernel matrix:

    >>> from yaplf.models.kernel import PrecomputedKernel
    >>> k = PrecomputedKernel(((9, 1, 4, 4), (1, 1, 1, 1), (4, 1, 4, 1), \
    ... (4, 1, 1, 4)))
    >>> from yaplf.data import get_precomputed_kernel_sample
    >>> labels = (-1, -1, 1, 1)
    >>> sample = get_precomputed_kernel_sample(labels, k)
    >>> from yaplf.algorithms.svm.classification import \
    ... SVMClassificationAlgorithm
    >>> from yaplf.algorithms.svm.solvers import CVXOPTClassificationSolver
    >>> s = CVXOPTClassificationSolver()
    >>> alg = SVMClassificationAlgorithm(sample, kernel = k, solver = s)
    >>> alg.run()
    >>> alg.model.test(sample)
    0.0
